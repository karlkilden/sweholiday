...
class SweHolidayCheckerTest {
    SweHolidayChecker hc;
    SweHolidayCheckerSpy scSpy;

    @BeforeEach
    void setUp() {
        hc = new SweHolidayChecker();
        scSpy = new SweHolidayCheckerSpy();
    }

    @Test
    void saturday() {
        assertThat(hc.isSweHoliday(LocalDate.of(2020, 06, 27))).isTrue();
    }

    @Test
    void sunday() {
        assertThat(hc.isSweHoliday(LocalDate.of(2020, 06, 28))).isTrue();
    }
    ...
    void skips_all_holidayFreeMonths() {
        for (Month m : SweHolidayChecker.HOLIDAY_FREE_MONTHS) {
    ...
    void not_weekendOrHoliday_sanity_test() {
        ...
        assertThat(hc.isSweHoliday(LocalDate.of(2020, 06, 30))).isFalse();
        assertThat(hc.isSweHoliday(LocalDate.of(2020, 04, 16))).isFalse();
        assertThat(hc.isSweHoliday(LocalDate.of(2020, 12, 17))).isFalse();
    }
    ...
    @Disabled
    void perf() {
        //CachingSweHolidayChecker chc = new CachingSweHolidayChecker(hc);
    ...
    private void assertIsHoliday(LocalDate date) {
        assertThat(hc.isSweHoliday(date)).isTrue();
    }

    private void assertMonthSkipped(Month month, int day) {
        scSpy.isSweHoliday(LocalDate.of(2020, month, day));
